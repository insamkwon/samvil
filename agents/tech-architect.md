...
2. **Folder structure**: standard Next.js 14 (app/, components/ui/, components/[feature]/, lib/ with store/types/utils/hooks)
3. **Data model**: TypeScript interfaces for all models, nanoid/uuid for IDs, ISO 8601 dates
4. **Prefer simplicity**: useState > Zustand > Redux, localStorage > database, Server Components default, API routes only if needed, no ORM for localStorage
5. **No over-engineering**: no microservices, no premature abstractions, no mixing App/Pages Router patterns
