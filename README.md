...
**그리고 받자마자 `npm test`가 돼요 (v4.31+).** 만들기만 하고 "잘 됐을 거예요"
하고 끝내지 않아요. 검증 과정이 결과물 안에 테스트로 남아서, 당신이 직접
`npm test`로 다시 확인할 수 있어요 — 빈 입력·새로고침·연타 같은 "쓰자마자
깨지는" 케이스까지 포함해서요.
...
```
  ...
  │       │       검증 과정이 결과물에 테스트로 남아요 (npm test)     │
  │       │       빈 입력·새로고침·연타 같은 엣지 케이스까지 커버     │
  │       │                                                          │
  │       ├── 실패 ──▶ 자동 수정 ──▶ 다시 ────────────────────────┐  │
  │       │                                                       │  │
  │       ▼                                                       │  │
...
| **검증 결과** | "잘 됐을 거예요" | 결과물에 테스트로 남아요 (`npm test`로 재확인) |
