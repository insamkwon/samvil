...
| **v4.4.0** | **3-Tier 헬스 분류** — healthy/degraded/critical 자동 감지 + `get_health_tier_summary` MCP 도구. |
| **v4.3.0** | **Domain Pack 강화** — game-phaser / webapp-enterprise 도메인 추가. |
| **v3.33.x** | **Consolidation** — 사용하지 않는 모듈/도구 정리, dead code 제거. |
| **v3.3.x** | **4-Layer Portability** — Codebase Manifest, Decision Log ADR, Orchestrator, HostCapability. 크로스호스트 체인 기반 구축. |
| **v3.2.0** | **Contract Layer** — Claim Ledger, 8 stage Gates, Model 라우팅, Jurisdiction, Retro v2, 성능 예산. 626 unit tests. |
| **v3.1.0** | **Interview Renaissance** — Deep Mode (Phase 2.6~2.9), Design stall 자동 복구, Auto-chain 기본 활성. |
| **v3.0.0** ⚠️ | **AC Tree Era (BREAKING)** — AC 트리 구조 도입. v2 프로젝트는 `/samvil:update --migrate`로 변환. PM Interview 모드 추가. |
...
1. **설치**: `/install-plugin insamkwon/samvil` 실행
