...
각 suggestion은 ISS-ID, severity, 대상 파일, 변경 내용, 근거를 포함:
...
Suggestions:
  1. ISS-01 [SEVERITY] <title>
     대상: <file path> (e.g., skills/samvil-build/SKILL.md)
     변경: <what to change>
     근거: <why — from metrics/events>
     기대효과: <expected improvement>

  2. ISS-02 [SEVERITY] <title>
     대상: <file path>
     변경: <what to change>
     근거: <why>
     기대효과: <expected improvement>

  3. ISS-03 [SEVERITY] <title>
     대상: <file path>
     변경: <what to change>
     근거: <why>
     기대효과: <expected improvement>
...
**SEVERITY 분류**:
- `CRITICAL`: 파이프라인 중단 원인 (build fail, QA 전체 실패)
...
```json
{
  ...
  "suggestions": [
    { "id": "ISS-01", "severity": "CRITICAL|HIGH|MEDIUM|LOW", "title": "...", "target_file": "...", "change": "...", "reason": "...", "expected_impact": "..." },
    { "id": "ISS-02", ... },
    { "id": "ISS-03", ... }
  ]
}
...
```json
{
  ...
  "suggestions": [
    { "id": "ISS-01", "severity": "<CRITICAL|HIGH|MEDIUM|LOW>", "title": "<title>", "target_file": "<file>", "change": "<what>", "reason": "<why>", "expected_impact": "<impact>" },
    { "id": "ISS-02", ... },
    { "id": "ISS-03", ... }
  ]}
```
...
Exactly 3 suggestions with ISS-ID, severity, target file, and structured rationale. All data from files — zero conversation-dependent metrics.
