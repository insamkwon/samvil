---
name: samvil-retro-legacy
description: "Legacy verbatim copy of samvil-retro/SKILL.md prior to the T4.1 ultra-thin migration. Reference only — not loaded by Claude."
---

# SAMVIL Retro — Self-Evolution Retrospective (legacy reference)

> NOTE: This is the pre-T4.1 (v3.35.x) full body, kept verbatim for
> reference. The active skill is `SKILL.md` (≤120 LOC). Pattern
> detection, metric aggregation, recurring-keyword scan, next-ID
> assignment, and MCP-health summary moved to
> `mcp/samvil_mcp/retro_aggregate.py` and the
> `mcp__samvil_mcp__aggregate_retro_metrics` tool. Pipeline-end
> Compressor narrate stays via `narrate_build_prompt` /
> `narrate_parse`. Suggestion choice + harness-feedback.log writes +
> AskUserQuestion preset accumulation stay in the skill body.
