---
name: samvil-orchestrator-legacy
description: "Legacy reference for the SAMVIL orchestrator entry point (preserved for v3.4 ultra-thin migration, T4.5). Do not invoke directly — see SKILL.md."
---

# SAMVIL Orchestrator — Legacy Reference (T4.5)

> **Status:** Preserved verbatim for the v3.4 ultra-thin migration. The
> active entry point is `skills/samvil/SKILL.md` (~150 LOC). This file
> retains the full Korean prose for Health Check, 3-layer solution_type
> detection, brownfield/resume routing, tier resolution, project init,
> Contract-Layer pre-flight, Gate A protocol, and Agent-Tier wiring.
>
> Consult only when the orchestrator regresses, when adding a new
> stage, or when changing the chain order. Aggregator inputs are pinned
> by `mcp/tests/test_samvil_smoke.py`.

# SAMVIL — Main Orchestrator (legacy body)
