---
name: samvil-qa-legacy
description: "[LEGACY] Verbatim QA prose for samvil-qa pre-T4.9 ultra-thin. Reference only. Active skill is samvil-qa."
---

> **Legacy reference (T4.9, 2026-04-25)**. The active `samvil-qa` skill
> is the ultra-thin version that delegates to `aggregate_qa_boot_context`,
> `dispatch_qa_pass1_batch`, `synthesize_qa_evidence`,
> `materialize_qa_synthesis`, and `finalize_qa_verdict` MCP tools.
> This file preserves the full per-`solution_type` Pass 1/1b/2/2.5/3
> bodies, Korean prose, anti-patterns, output format, and Ralph loop
> details for situations where the thin skill needs a verbatim
> reference (e.g., MCP unreachable, novel solution_type).

# SAMVIL QA — 3-Pass Verification (LEGACY)
