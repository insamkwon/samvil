...
3. `mcp__samvil_mcp__gate_check(gate_name="build_to_qa", samvil_tier=<tier>, metrics_json=<gate_input>, project_root=".", evidence_mode="mechanical")`. Mandatory: unavailable/error → halt. On block, record the `gate_verdict`, AskUserQuestion, and halt unless the user explicitly approves; only then call `mcp__samvil_mcp__gate_override(project_root=".", gate="build_to_qa", reason="<user-approved reason>", approved_by="user")` and proceed. `verdict=pass` → record `gate_verdict` + proceed.
