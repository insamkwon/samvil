---
name: samvil-build
description: "Build core experience + features from seed. Circuit breaker per feature. Context Kernel sync on every step. Gate blocks require explicit user approval plus gate_override; unapproved force_proceed is forbidden."
---
...
3. `mcp__samvil_mcp__gate_check(gate_name="build_to_qa", samvil_tier=<tier>, metrics_json=<gate_input>, project_root=".", evidence_mode="mechanical")`. Mandatory: unavailable/error → halt. On block, record the `gate_verdict`, AskUserQuestion, and halt unless the user explicitly approves. Proceed only when the host supplies an immutable `user_approval` claim ID; then call `mcp__samvil_mcp__gate_override(project_root=".", gate="build_to_qa", reason="<exact user-approved reason>", approval_claim_id="<host-issued claim ID>")`. If the host cannot issue that claim, halt. `verdict=pass` → record `gate_verdict` + proceed.
...
2. **TaskUpdate** "Build" → `completed`. Print `[SAMVIL] Stage 5/5: Running QA verification...`.
