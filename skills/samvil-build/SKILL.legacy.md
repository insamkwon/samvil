---
name: samvil-build-legacy
description: "[LEGACY — preserved for reference only] Pre-T4.8 monolithic build skill. The active skill is `samvil-build` (ultra-thin) which delegates to MCP aggregators `aggregate_build_phase_a`, `dispatch_build_batch`, `finalize_build_phase_z`. Consult this file for verbatim per-`solution_type` Korean prose, code-quality rules, anti-patterns, and full Phase A/B/Z bodies that pre-date the contract-layer split."
---

# SAMVIL Build — Core Experience + Feature Implementation (LEGACY, pre-T4.8)
<!-- LEGACY-INDEX:BEGIN (generated by scripts/gen-legacy-index.py — do not edit) -->
...
| Line | Section |
|---:|---|
| 81 | Boot Sequence (INV-1) |
| 106 | Structured Build Event Schema |
| 118 | Phase A — pre_stage Contract Layer (v3.2.0, ④+⑥+⑤) |
| 149 | Phase A: Core Experience |
| 151 | &nbsp;&nbsp;↳ solution_type: "web-app" |
| 205 | &nbsp;&nbsp;↳ solution_type: "game" |
| 231 | &nbsp;&nbsp;↳ solution_type: "automation" |
| 289 | &nbsp;&nbsp;↳ solution_type: "mobile-app" |
| 321 | &nbsp;&nbsp;↳ solution_type: "dashboard" |
| 409 | Phase A.5: Dependency Pre-Resolution |
| 413 | &nbsp;&nbsp;↳ web-app |
| 421 | &nbsp;&nbsp;↳ automation |
| 438 | Phase A.6: Scaffold Sanity Check (v3.1.0, v3-026) |
| 444 | &nbsp;&nbsp;↳ A.6.1 Key config files must be non-empty |
| 465 | &nbsp;&nbsp;↳ A.6.2 No unsubstituted template variables |
| 477 | &nbsp;&nbsp;↳ A.6.3 Import paths point to files that exist |
| 511 | &nbsp;&nbsp;↳ A.6.4 Log + continue |
| 528 | Phase A ↔ Phase B boundary (v3.0.0) |
| 532 | Phase B Mode Selection (v3.0.0+) |
| 553 | Phase B-Tree: AC Tree Execution (v3.0.0+) |
| 557 | &nbsp;&nbsp;↳ Step 1: Parse AC tree |
| 569 | &nbsp;&nbsp;↳ Step 2: Resume from checkpoint (if any) |
| 597 | &nbsp;&nbsp;↳ Step 2.5: Dependency Planning (v3.0.0 T2, optional) |
| 642 | &nbsp;&nbsp;↳ Step 3: Tree loop (until `tree_progress.all_done` is true) |
| 767 | &nbsp;&nbsp;↳ Step 4: Integration build per feature |
| 771 | &nbsp;&nbsp;↳ Step 5: Persist tree into seed |
| 775 | &nbsp;&nbsp;↳ Step 6: Progress report |
| 793 | &nbsp;&nbsp;↳ Parallelism rules |
| 798 | &nbsp;&nbsp;↳ Rate budget lifecycle (summary) |
| 821 | &nbsp;&nbsp;↳ Backward compatibility |
| 826 | Phase B: Features |
| 837 | &nbsp;&nbsp;↳ solution_type: "automation" — Module-based Build |
| 858 | &nbsp;&nbsp;↳ solution_type: "game" — Mechanic-based Build |
| 889 | &nbsp;&nbsp;↳ solution_type: "mobile-app" — Screen-based Build |
| 921 | &nbsp;&nbsp;↳ solution_type: "dashboard" — Chart-based Build |
| 953 | &nbsp;&nbsp;↳ solution_type: "web-app" — Component-based Build (기존) |
| 955 | &nbsp;&nbsp;↳ Step 1: Classify Features into Batches |
| 967 | &nbsp;&nbsp;↳ Step 2: Build Each Batch |
| 1080 | &nbsp;&nbsp;↳ Dynamic Parallelism (MAX_PARALLEL) |
| 1105 | &nbsp;&nbsp;↳ Independence Verification (병렬 가능 여부 체크) |
| 1128 | &nbsp;&nbsp;↳ Worker Context Budget (토큰 최적화) |
| 1174 | &nbsp;&nbsp;↳ Chunk Dispatch |
| 1220 | &nbsp;&nbsp;↳ Progress Trim (실시간 진행 상황 출력) |
| 1273 | &nbsp;&nbsp;↳ After All Features |
| 1332 | Phase Z — post_stage Contract Layer (v3.2.0, ①+⑥) |
| 1380 | Chain to QA (INV-4) |
| 1386 | &nbsp;&nbsp;↳ Handoff Write |
...
> Reference-only snapshot of the 1432-LOC build skill body that shipped
> through v3.35.x. The active skill `samvil-build/SKILL.md` is now an
> ultra-thin (≤120 active LOC) wrapper that calls three MCP aggregators
> for boot/dispatch/finalize. Use this file when:
> - extending or debugging a per-`solution_type` build template
> - looking up the verbatim Korean prompt copy
> - cross-checking the original Phase B parallel dispatch contract
> Do not edit this file in place; update the active skill or the
> `mcp/samvil_mcp/build_phase_*.py` modules instead.
