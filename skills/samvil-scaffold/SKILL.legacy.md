---
name: samvil-scaffold-legacy
description: "Reference body preserved from pre-T4.7 samvil-scaffold (1653 LOC). Active skill is now ultra-thin SKILL.md backed by mcp__samvil_mcp__evaluate_scaffold_target."
---

> **LEGACY — preserved for reference only.** The active skill is
> `SKILL.md` in this directory, which delegates the per-stack CLI command
> catalog, dependency-matrix lookup, sanity checks, and build-artifact
> rules to `mcp__samvil_mcp__evaluate_scaffold_target` (T4.7,
> 2026-04-25). Consult this file only when the scaffold is failing or
> being extended for a new framework — for example, adding Nuxt or
> SvelteKit support, or chasing a regression in the Tailwind v3/v4
> overwrite dance after `shadcn init`.
